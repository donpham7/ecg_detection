...
class ECGCNN_MoE(nn.Module):
    def __init__(
        ...
        self.dropout1 = nn.Dropout(p=0.05)  # Could be 0.03 0.05 0.1
        self.dropout2 = nn.Dropout(p=0.03)  # Could be 0.03 0.05 0.1
...
class ECGCNN_MoE_Small(nn.Module):
    def __init__(
        ...
        self.router_pooling = nn.AdaptiveAvgPool1d(1)
        self.router_noise_std = 0.03
        self.router = nn.Linear(8, num_experts)

        # Calculate dimensions correctly
        # After conv1: (batch, 8, 187)
        # After expert blocks: (batch, 32, 187//4) = (batch, 32, 46)
        final_length = (input_length // 4) * 32  # 46 * 32 = 1472

        self.fc1 = nn.Linear(final_length, 64)
        self.fc2 = nn.Linear(64, num_classes)
        self.dropout = nn.Dropout(p=0.1)
    ...
    def forward(self, x, return_aux_loss=True):
        ...
        # Process experts
        expert_outputs = torch.zeros(batch_size, 32, x.size(2) // 4, device=self.device)
